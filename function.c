...
int OptionUiNum1() {
	...
	printf("5.일본 - 도쿄(NRT)               2022/06/03/13:10      대한한공\n\n");
	printf("6.일본 - 도쿄(NRT)               2022/06/08/15:35      대한항공\n\n");
	printf("7.일본 - 도쿄(NRT)               2022/06/17/20:40      진에어\n\n");
	printf("8.일본 - 도쿄(NRT)               2022/06/30/07:50      진에어\n\n");
Back:
	printf("입력 : ");
	scanf("%d", &selectNum);
	printf("------------------------------------------------------------------------------------\n");
...
void selectMenu() {
	while (1) {
		...
		if (menuCode == 0) {
			...
			Return0:
				system("cls");
				printf("이미 예약한 좌석이 있습니다. 좌석 삭제후 선택해주세요.\n");
				printf("스페이스바를 입력하면 메인메뉴로 이동합니다.\n");
				int key = getch();
				if (key == 32) {
					fclose(fp);
					system("cls");
